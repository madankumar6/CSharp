...
{
    public class Gt100Protocol : Protocol
    {
        public struct ProtocolNumber
        {
            public const string LoginMessage = "01";
            public const string LocationData = "22";
            public const string StatusInformation = "13";
            public const string StringInformation = "21";
            public const string AlarmData = "26";
            public const string TimeZoneNTime = "27";
            public const string GPSQueryAddress = "2A";
            public const string CommandInformationToTerminal = "80";
        }

        public struct PayloadLength
        {
            public const int LoginMessage = 22;
            public const int LocationData = 39;
            public const int AlarmData = 42;
            public const int StatusInformation = 15;

            //int _StringInformation = 21;
            //int _TimeZoneNTime = 27;
            //int _GPSQueryAddress = 0;
            //int _CommandInformationToTerminal = 80;
        }
        ...
        public override DeviceInfo Parse(DeviceInfo deviceInfo)
        {
            try
            {
                // It cannot be moved to common, Each protocol can have their own String convertion
                ...
                if (ParserHelper.TSubstring(ref deviceInfo.Payload, 0, 4) == "7878")
                {
                    ...
                    else if (packetComparer == ProtocolNumber.LocationData)
                    {
                        if (tPayload.Length >= (PayloadLength.LocationData * 2))
                        {
                            ...
                            var Latitude = (latMin / ((double)60) / ((double)30000)).ToString("0.000000000").Replace("-", ""); // To device Minute value

                            var longMin = (Double)Convert.ToInt32(ParserHelper.TSubstring(ref deviceInfo.Payload, 0, 8), 16);
                            var Longitude = (longMin / ((double)60) / ((double)30000)).ToString("0.000000000").Replace("-", ""); // To device Minute value
                            ...
                            var datauploadmode = ParserHelper.TSubstring(ref deviceInfo.Payload, 0, 2);
                            var gpsrealtimeupload = ParserHelper.TSubstring(ref deviceInfo.Payload, 0, 2);
                            ...
                            object obj = new
                            {
                                ...
                                gpsrealtimeupload = gpsrealtimeupload
                            };
                    ...
                    else if (packetComparer == ProtocolNumber.StatusInformation)
                    {
                        if (tPayload.Length >= (PayloadLength.StatusInformation * 2))
                        {
                            ...
                            //var tic = deviceInfo.Payload.Substring(0, 2);
                            string statusInfo = ParserHelper.TSubstring(ref deviceInfo.Payload, 0, 10);
                    ...
                    else
                    {
                        ...
                        // Skip the length of this message type 
